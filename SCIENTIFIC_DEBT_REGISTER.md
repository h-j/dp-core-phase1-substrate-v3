...
| **SD-007** | **Legacy Keyword Confidence Engine Downgrade** | All confidence-evolution results produced prior to Scored Confidence Engine integration (PROMPT R1) used LLM text keyword scans ("strengthen", "uncertain", etc.) to update confidence. These results are superseded for major scientific purposes. | Claims based on pre-PROMPT-R1 confidence evolution runs (claim maturity downgrade applied per governance table). | Replaced engine with `ScoredConfidenceEngine` driven by Beta posterior over predicate validation outcomes, staleness decay, and contradiction counts. |
