...
def main():
    ...
    sample_index = market_observation_repository.count() % len(
        SAMPLE_NIFTY_OBSERVATIONS
    )

    market_observation = SAMPLE_NIFTY_OBSERVATIONS[sample_index]

    market_regime = market_regime_classifier.classify(market_observation)
    ...
    abstraction = abstraction_flow.process(observation)
    ...
    recent_reflective_memories = reflective_memory_repository.list_recent(limit=1)

    reflective_memory_summary = ""

    if recent_reflective_memories:
        reflective_memory_summary = recent_reflective_memories[
            0
    ...
    validation = ValidationEvent(
        ...
        observed_behavior=(market_observation.observation_text),
        expected_behavior=(
    ...
    reflection = reflection_flow.process(theory, validation)
    ...
    outcome_index = market_outcome_repository.list_recent(limit=1).__len__() % len(
    ...
    outcome_validation_result = outcome_validation_engine.validate(
        theory=theory, prior_observation=observation, market_outcome=market_outcome
    )

    print("OUTCOME VALIDATION COMPLETED")
    print(f"VALIDATION SCORE: {outcome_validation_result['validation_score']:.2f}")
    print(
        f"CONTRADICTIONS DETECTED: {outcome_validation_result['contradictions_detected']}"
    )
    ...
    theory_survival_tracker.track(
        theory=theory, validation_result=outcome_validation_result
    )

    theory_survival_analysis = theory_survival_tracker.analyze_trends()
    theory_survival_summary = theory_survival_tracker.generate_survival_summary()
    ...
    contradiction_zone_map = market_contradiction_mapper.map_recurring_zones()
    contradiction_map_summary = market_contradiction_mapper.generate_contradiction_map()
    ...
    print("EMPIRICAL: " f"{evolved_confidence_state.empirical_confidence}")
    print("REGIME: " f"{evolved_confidence_state.regime_confidence}")
    print("REFLECTION: " f"{evolved_confidence_state.reflection_confidence}")
    print("COHERENCE: " f"{evolved_confidence_state.theoretical_coherence}")
    print(
    ...
    for recent_outcome in recent_outcomes:
        ...
        outcome_validation_results.append(outcome_validation_result)
    ...
    reflective_memory_state = reflective_memory_synthesizer.synthesize(
        ...
        contradiction_result=contradiction_result,
        market_observations=(market_observation_repository.list_recent(limit=10)),
    ...
    print("TRAJECTORY: " f"{reflective_memory_state.cognition_trajectory_summary}")
    print("HOTSPOTS: " f"{'; '.join(reflective_memory_state.contradiction_hotspots)}\n")
    ...
    strategic_memory_state = strategic_reflection_synthesizer.synthesize(
        ...
        contradiction_zone_map=contradiction_zone_map,
        market_observations=(market_observation_repository.list_recent(limit=10)),
        reflective_memory_states=(reflective_memory_repository.list_recent(limit=5)),
    )

    strategic_memory_repository.save(strategic_memory_state)

    print("STRATEGIC MEMORY PERSISTED")
    print("SUMMARY: " f"{strategic_memory_state.strategic_summary}")
    print("POSTURE: " f"{strategic_memory_state.cognition_posture}\n")
    ...
    cognition_narrative = cognition_narrative_builder.build_narrative(
        ...
        recent_outcomes=recent_outcomes,
        theory_survival_analysis=theory_survival_analysis,
        strategic_memory_history=(strategic_memory_repository.list_recent(limit=5)),
    )
    ...
    print(
        reflective_query_engine.query_coherence_decline(
            recent_confidence_states=recent_confidence_states,
            recent_reflections=recent_reflections,
            theory_survival_analysis=theory_survival_analysis,
        )
    ...
    print()
    print(
        reflective_query_engine.query_repeated_failures(
            theory_survival_analysis=theory_survival_analysis,
            recent_outcomes=recent_outcomes,
        )
    )
    print()
    print(
        reflective_query_engine.query_contradiction_frequency(
            contradiction_zone_map=contradiction_zone_map,
            recent_reflections=recent_reflections,
        )
    )
    print()
    print(
        reflective_query_engine.query_regime_sensitivity(
            theory_survival_analysis=theory_survival_analysis,
            recent_confidence_states=recent_confidence_states,
            recent_outcomes=recent_outcomes,
        )
    ...
    print()
    print(
        reflective_query_engine.query_weakening_assumptions(
            theory_survival_analysis=theory_survival_analysis,
            recent_validations=recent_validations,
        )
    )
