...
def generate_mock_experiences(
    belief_record: Dict[str, Any], lift: float, size: int = 200
) -> List[Dict[str, Any]]:
    """
...
def run_evolution_experiment():
    ...
    state_d1_a2 = evaluate_longitudinal_evidence(
        runner_d1, belief_d1, e_strong_pos, "T2_A"
    )
    ...
    state_d2_b1 = evaluate_longitudinal_evidence(
        runner_d2, belief_d2, e_strong_pos2, "T1_B"
    )
    ...
    assert (
        belief_e["evolution_history"][1]["from_state"] == LifecycleState.ADMITTED_BELIEF
    )
    assert (
        belief_e["evolution_history"][1]["to_state"] == LifecycleState.ADMITTED_BELIEF
    )
    ...
    assert (
        g1["status"] == "PASS"
    ), f"Milestone 6 Gate 1 Temporal Isolation Violation: {g1['evidence']}"
    assert (
        g7["status"] == "PASS"
    ), f"Milestone 6 Gate 7 ERC Authorization Violation: {g7['evidence']}"
    assert (
        g8["status"] == "PASS"
    ), f"Milestone 6 Gate 8 Immutability Violation: {g8['evidence']}"
