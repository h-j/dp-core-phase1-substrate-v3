...
def test_structured_lesson_record():
    ...
    assert reconstructed.activation_conditions == ["volatility: compressed", "momentum: strengthening"]
...
def test_weighted_similarity_retrieval():
    ...
    lesson_a = LessonRecord(
        lesson_id=uuid4(),
        target_regime={"regime_subtype": "momentum", "volatility": "compressed", "momentum": "strengthening", "volume": "normal"},
        status=LessonStatus.ACTIVE,
    ...
    lesson_b = LessonRecord(
        lesson_id=uuid4(),
        target_regime={"regime_subtype": "momentum", "volatility": "expanded", "momentum": "flat", "volume": "high"},
        status=LessonStatus.ACTIVE,
    ...
    lesson_c = LessonRecord(
        lesson_id=uuid4(),
        target_regime={"regime_subtype": "range_bound", "volatility": "compressed", "momentum": "strengthening", "volume": "high"},
        status=LessonStatus.ACTIVE,
    ...
    assert scored_lessons[0][0].lesson_text == "Lesson A" # Perfect match -> 1.0 similarity
    assert abs(scored_lessons[0][1] - 1.0) < 0.001
    
    assert scored_lessons[1][0].lesson_text == "Lesson C" # Volatility (0.3) + Momentum (0.2) match -> 0.5 similarity
    assert abs(scored_lessons[1][1] - 0.5) < 0.001
    
    assert scored_lessons[2][0].lesson_text == "Lesson B" # Subtype match (0.4) only -> 0.4 similarity
