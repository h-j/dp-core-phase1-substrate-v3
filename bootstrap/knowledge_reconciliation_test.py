...
class TestKnowledgeReconciliation(unittest.TestCase):
    ...
    @patch(
        "interfaces.ollama_client.OllamaClient.generate", side_effect=mock_llm_generate
    )
    ...
    def test_hierarchical_merging(self, mock_gen):
        ...
        reconciled_actives = [
            p for p in merged_p if p.status != PrincipleStatus.RETIRED
        ]
        self.assertEqual(len(reconciled_actives), 1)
        self.assertEqual(
            reconciled_actives[0].statement,
            "Consolidated broad volume confirmation mechanism.",
        )
    ...
    def test_knowledge_reconciliation_engine(self, mock_gen):
        ...
        stable_principles = [
            p for p in reconciled if p.status == PrincipleStatus.CANONICAL
        ]
        self.assertEqual(len(stable_principles), 1)
        self.assertEqual(
            stable_principles[0].statement,
            "Broad generalized volume confirmation mechanism statement.",
        )
