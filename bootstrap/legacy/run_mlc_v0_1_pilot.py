...
def run_pilot():
    ...
    artifact_dir = os.path.join(
        project_root, "artifacts", "experiments", "mlc_v0_1_pilot", run_id
    )
    ...
    print(
        f"Registry frozen and hashed. World count: {len(registry)}. Hash: {registry_hash}"
    )
        ...
        ground_truth.append(
            {
                ...
                "expected_reason": entry["ground_truth_defer_subtype"]
                or (
                    "SUFFICIENT_POSITIVE_PROSPECTIVE_EFFECT"
                    if entry["true_effect"] > 0
    ...
    for idx, entry in enumerate(registry):
        ...
        if (idx + 1) % 10 == 0 or idx == 0:
            print(
                f"  [{idx + 1}/100] Running lifecycle for {entry['world_id']} (Zone: {entry['zone']})"
            )
    ...
    h5_metrics = MLCPilotMetrics.calculate_h5_metrics(mlc_decisions, ground_truth)
    ...
    for entry in registry:
        ...
        mlc_correct = (
            1
            if mlc_map.get(w_id) == oracle_map.get(w_id)
            and mlc_map.get(w_id) != "COMPILATION_REJECTED"
            else 0
        )
        b2_correct = (
            1
            if b2_map.get(w_id) == oracle_map.get(w_id)
    ...
    protocol_info = {
        ...
        "pilot_size": 100,
        "primary_size": 500,
        "zone_composition": {"clear": 30, "boundary": 50, "evidence_limited": 20},
    ...
    gates_passed = all(
        g["status"] == "PASS"
        for name, g in gates_res.items()
        if name != "PILOT_GATE_8_ARTIFACT_COMPLETENESS"
    )

    if not gates_passed:
        # Determine specific failure
        if (
            gates_res["PILOT_GATE_1_WORLD_COUNT"]["status"] == "FAIL"
            or gates_res["PILOT_GATE_2_ZONE_COMPOSITION"]["status"] == "FAIL"
        ):
...
"""
    for g_name, g_val in gates_res.items():
        ...
        status = (
            "PASS"
            if g_name == "PILOT_GATE_8_ARTIFACT_COMPLETENESS"
            else g_val["status"]
        )
...
"""
    ...
    expected_filenames = [
        "pilot_protocol.json",
        "pilot_world_registry.json",
        "pilot_world_registry_hash.json",
        "pilot_ground_truth.json",
        "pilot_results.csv",
        "pilot_mlc_decisions.json",
        "pilot_baseline_results.json",
        "pilot_pooled_metrics.json",
        "pilot_zone_metrics.json",
        "pilot_h4_decomposition.json",
        "pilot_h5_metrics.json",
        "pilot_paired_comparisons.json",
        "pilot_power_analysis.json",
        "pilot_validity_gates.json",
