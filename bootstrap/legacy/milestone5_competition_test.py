...
def test_runner_with_competition_and_erc():
    ...
    print(
        "✓ MLCExperimentRunner competition run and budget tracking verified successfully."
    )
...
def test_selection_risk_and_prospective_validation():
    ...
    dec_b = runner_b.run_lifecycle_with_competition(
        world, num_confounders=5, enable_prospective_filter=False
    )
    ...
    winner_retro_metrics = next(
        m for m in runner_b.intrinsic_measurements if m["proposition_id"] == winner_b
    )
    winner_prospective_metrics = next(
        m for m in runner_b.prospective_measurements if m["proposition_id"] == winner_b
    )
    optimism = (
        winner_retro_metrics["comparative_effect"]
        - winner_prospective_metrics["comparative_effect"]
    )
    ...
    dec_c = runner_c.run_lifecycle_with_competition(
        world, num_confounders=5, enable_prospective_filter=True
    )
    ...
    print(
        "✓ Selection Risk (Selection Optimism) and Prospective Validation protection verified on seed 3."
    )
