...
def run_comparison():
    ...
    snapshot_path = Path(
        "/Users/hemantj/Proj/dp_core/dp-core-phase1-substrate-v3/data/replay_snapshots/reliance/run_20260701_123929/day_0000.json"
    )
    ...
    try:
        ...
        print(
            ...
            len(baseline_theory.summary_structured.mechanism_components),
        )
    ...
    comp_data = {
        "baseline": (
            {
                "claim": (
                    baseline_theory.summary_structured.claim
                    if baseline_theory
                    else None
                ),
                "mechanism": (
                    baseline_theory.summary_structured.mechanism
                    if baseline_theory
                    else None
                ),
                "components": (
                    [
                        c.to_dict()
                        for c in baseline_theory.summary_structured.mechanism_components
                    ]
                    if baseline_theory
                    else []
                ),
        ...
        "strategy_b": (
            {
                ...
                "mechanism": (
                    b_theory.summary_structured.mechanism if b_theory else None
                ),
                "components": (
                    [
                        c.to_dict()
                        for c in b_theory.summary_structured.mechanism_components
                    ]
                    if b_theory
                    else []
                ),
                "trigger_definition": (
                    b_theory.summary_structured.trigger_definition if b_theory else None
                ),
                "target_definition": (
                    b_theory.summary_structured.target_definition if b_theory else None
                ),
                "expected_direction": (
                    b_theory.summary_structured.expected_direction if b_theory else None
                ),
                "contradiction_definition": (
                    b_theory.summary_structured.contradiction_definition
                    if b_theory
                    else None
                ),
                "driver": b_theory.summary_structured.driver if b_theory else None,
                "mediator_or_process": (
                    b_theory.summary_structured.mediator_or_process
                    if b_theory
                    else None
                ),
                "target_effect": (
                    b_theory.summary_structured.target_effect if b_theory else None
                ),
            }
            if b_theory
            else None
    ...
    output_path = Path(
        "/Users/hemantj/.gemini/antigravity-ide/brain/face4af3-acf5-4ebb-a5bd-ccb6af890aa2/strategy_b_comparison_results.json"
    )
