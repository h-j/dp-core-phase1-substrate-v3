...
class OperationalHypothesisHarness:
    def __init__(
        ...
        self.artifact_path = Path(
            artifact_path
            or (Path(__file__).resolve().parents[1] / "data" / "nifty_daily_3y.csv")
        )
        self.output_dir = Path(
            ...
            or (
                ...
                / "data"
                / "operational_hypothesis_experiment"
            )
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for candidate in candidate_pool:
            validation = self.validate_candidate(
                candidate, formation_window, seen_candidates
            )
        ...
        for hypothesis in frozen:
            for day_index in range(len(evaluation_window)):
                evidence_history.append(
                    self._evaluate_hypothesis_on_day(hypothesis, day_index, artifact)
                )

        formation_metrics = self._build_formation_metrics(
            candidate_pool, rejections, accepted
        )
        evaluation_metrics = self._build_evaluation_metrics(evidence_history)
        per_hypothesis_reports = self._build_per_hypothesis_reports(
            frozen, evidence_history
        )
        base_rate_diagnostics = self._build_base_rate_diagnostics(
            frozen, evaluation_window, evidence_history
        )
    ...
    def _generate_candidate_pool(
        self, formation_window: pd.DataFrame
    ) -> List[Dict[str, Any]]:
        candidates = []
    ...
    def validate_candidate(
        ...
        if not self._is_normalizable_predicate(candidate.get("trigger_predicate")):
            return {
                ...
                "source_provenance": candidate.get("source_provenance"),
                "status": "rejected",
                "rejection_reason": "INVALID_PREDICATE",
            }
        ...
        if not self._is_normalizable_predicate(
            candidate.get("expected_effect_predicate")
        ):
            return {
                "candidate": candidate,
                "source_provenance": candidate.get("source_provenance"),
        ...
        if not self._is_allowed_operator(
            candidate.get("trigger_predicate", {}).get("operator")
        ):
            return {
                "candidate": candidate,
                "source_provenance": candidate.get("source_provenance"),
        ...
        if not self._is_allowed_operator(
            candidate.get("expected_effect_predicate", {}).get("operator")
        ):
            return {
                "candidate": candidate,
                "source_provenance": candidate.get("source_provenance"),
        ...
        if not self._is_positive_horizon(candidate.get("evaluation_horizon")):
            return {
                ...
                "source_provenance": candidate.get("source_provenance"),
                "status": "rejected",
                "rejection_reason": "INVALID_HORIZON",
            }
        ...
        if self._is_same_observable_restatement(candidate):
            return {
                ...
                "source_provenance": candidate.get("source_provenance"),
                "status": "rejected",
                "rejection_reason": "SAME_OBSERVABLE",
            }
    ...
    def _is_normalizable_predicate(self, predicate: Optional[Dict[str, Any]]) -> bool:
        ...
        if not isinstance(predicate["field"], str) or not predicate["field"]:
            return False
        if (
            not isinstance(predicate["operator"], str)
            or predicate["operator"] not in ALLOWED_OPERATORS
        ):
            return False
        return True
    ...
    def _contains_tautology(self, candidate: Dict[str, Any]) -> bool:
        for predicate in [
            candidate.get("trigger_predicate"),
            candidate.get("expected_effect_predicate"),
        ]:
            if isinstance(predicate, dict) and predicate.get("field") == predicate.get(
                "value"
            ):
                return True
        return False
    ...
    def _expected_effect_known_at_formation(
        self, candidate: Dict[str, Any], formation_window: pd.DataFrame
    ) -> bool:
    ...
    def _canonical_key(self, candidate: Dict[str, Any]) -> Tuple[Tuple[Any, ...], ...]:
        ...
        scopes = tuple(
            sorted(
                ...
                for pred in candidate.get("scope_predicates", [])
            )
        )
    ...
    def _evaluate_hypothesis_on_day(
        self, hypothesis: Dict[str, Any], day_index: int, artifact: pd.DataFrame
    ) -> Dict[str, Any]:
        row = artifact.iloc[day_index + 10]
        scope_result = self._evaluate_scope(
            hypothesis.get("scope_predicates", []), day_index + 10, artifact
        )
        trigger_result = self._evaluate_predicate(
            hypothesis.get("trigger_predicate"), day_index + 10, artifact
        )
        effect_result = None
        if scope_result["result"] == "FALSE":
            ...
            if trigger_result["result"] == "FALSE":
                ...
                target_day = compute_target_day(
                    activation_day=activation_day,
                    horizon=hypothesis.get("evaluation_horizon", 0),
                )
                if (
                    target_day is None
                    or target_day < EVALUATION_WINDOW_START
                    or target_day > EVALUATION_WINDOW_END
                ):
                    evidence_outcome = "INCONCLUSIVE"
                    reason_code = "TARGET_OUTSIDE_EVALUATION_WINDOW"
                else:
                    effect_result = self._evaluate_predicate(
                        hypothesis.get("expected_effect_predicate"),
                        target_day - 1,
                        artifact,
                    )
        ...
        target_row = (
            artifact.iloc[target_day - 1]
            if target_day is not None and 1 <= target_day <= len(artifact)
            else None
        )
        return {
            ...
            "activation_status": (
                "ACTIVATED"
                if scope_result["result"] == "TRUE"
                and trigger_result["result"] == "TRUE"
                else "NOT_ACTIVATED"
            ),
            "target_day": (
                target_day
    ...
    def _evaluate_scope(
        self, predicates: List[Dict[str, Any]], index: int, artifact: pd.DataFrame
    ) -> Dict[str, Any]:
        if not predicates:
            return {
                "result": "TRUE",
        ...
        results = [
            self._evaluate_predicate(predicate, index, artifact)
    ...
    def _evaluate_predicate(
        self, predicate: Optional[Dict[str, Any]], index: int, artifact: pd.DataFrame
    ) -> Dict[str, Any]:
        if not predicate:
            return {
                "result": "UNKNOWN",
        ...
        return {
            "result": "TRUE" if compare else "FALSE",
            "field": field,
            "operator": operator,
            "value": value,
            "actual": row_value,
    ...
    def _build_formation_metrics(
        ...
        for rejection in rejections:
            counts[rejection["rejection_reason"]] = (
                counts.get(rejection["rejection_reason"], 0) + 1
            )
        return {
    ...
    def _build_evaluation_metrics(
        ...
        metrics = {
            ...
            "not_applicable_count": sum(
                1 for item in history if item["evidence_outcome"] == "NOT_APPLICABLE"
            ),
            "activation_count": sum(
                1 for item in history if item["activation_status"] == "ACTIVATED"
            ),
            "tested_count": sum(
                1
                for item in history
                if item["evidence_outcome"] in {"SUPPORTED", "CONTRADICTED"}
            ),
            "supported_count": sum(
                1 for item in history if item["evidence_outcome"] == "SUPPORTED"
            ),
            "contradicted_count": sum(
                1 for item in history if item["evidence_outcome"] == "CONTRADICTED"
            ),
            "inconclusive_count": sum(
                1 for item in history if item["evidence_outcome"] == "INCONCLUSIVE"
            ),
            "inconclusive_counts_by_reason": {},
        }
        for item in history:
            if item["evidence_outcome"] == "INCONCLUSIVE" and item.get("reason_code"):
                metrics["inconclusive_counts_by_reason"][item["reason_code"]] = (
                    metrics["inconclusive_counts_by_reason"].get(item["reason_code"], 0)
                    + 1
                )
        return metrics

    def _build_per_hypothesis_reports(
        self, hypotheses: List[Dict[str, Any]], history: List[Dict[str, Any]]
        ...
        for hypothesis in hypotheses:
            items = [
                item
                for item in history
                if item["hypothesis_id"] == hypothesis.get("hypothesis_id")
            ]
            tested = [
                item
                for item in items
            ...
            reports[hypothesis.get("hypothesis_id")] = {
                ...
                "activations": sum(
                    1 for item in items if item["activation_status"] == "ACTIVATED"
                ),
                "tested_experiences": len(tested),
                "supported": sum(
                    1 for item in tested if item["evidence_outcome"] == "SUPPORTED"
                ),
                "contradicted": sum(
                    1 for item in tested if item["evidence_outcome"] == "CONTRADICTED"
                ),
                "inconclusive": sum(
                    1 for item in items if item["evidence_outcome"] == "INCONCLUSIVE"
                ),
                "not_applicable": sum(
                    1 for item in items if item["evidence_outcome"] == "NOT_APPLICABLE"
                ),
                "support_ratio_among_tested_experiences": (
                    (
                        sum(
                            1
                            for item in tested
                            if item["evidence_outcome"] == "SUPPORTED"
                        )
                        / len(tested)
                    )
                    if tested
    ...
    def _build_base_rate_diagnostics(
        self,
        ...
        for hypothesis in hypotheses:
            ...
            support_ratio = (
                (
                    sum(1 for item in tested if item["evidence_outcome"] == "SUPPORTED")
                    / len(tested)
                )
                if tested
                else None
            ...
            if len(evaluation_window) > 0:
                target_base_rate = (
                    len(tested) / len(evaluation_window)
                    if len(evaluation_window)
                    else None
                )
            diagnostics[hypothesis.get("hypothesis_id")] = {
                "support_ratio": support_ratio,
                "unconditional_target_base_rate_over_evaluation_window": target_base_rate,
                "descriptive_support_lift": (
                    (support_ratio - target_base_rate)
                    if support_ratio is not None and target_base_rate is not None
                    else None
                ),
            }
        return diagnostics

    def _build_frozen_hypothesis_report(
    ...
    def _write_outputs(self, report: Dict[str, Any]) -> None:
        with (self.output_dir / "frozen_hypothesis_report.json").open(
            "w", encoding="utf-8"
        ) as handle:
            json.dump(report["frozen_hypothesis_report"], handle, indent=2)
        with (self.output_dir / "rejection_report.json").open(
            "w", encoding="utf-8"
        ) as handle:
            json.dump(report["rejections"], handle, indent=2)
        with (self.output_dir / "raw_evaluation_report.json").open(
            "w", encoding="utf-8"
        ) as handle:
            json.dump(report["history"], handle, indent=2)
        with (self.output_dir / "summary_report.json").open(
            "w", encoding="utf-8"
        ) as handle:
            json.dump(
                {
        ...
        with (self.output_dir / "frozen_hypothesis_report.md").open(
            "w", encoding="utf-8"
        ) as handle:
...
def compute_target_day(activation_day: int, horizon: int) -> Optional[int]:
    if (
        not isinstance(activation_day, int)
