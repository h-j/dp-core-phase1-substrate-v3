...
class TestBenchmarkRunner(unittest.TestCase):
    ...
    def test_benchmark_runner_e2e(self, mock_gen):
        ...
        with patch.object(BenchmarkRunner, "__init__", mock_init):
            runner = BenchmarkRunner(assets_list=["RELIANCE", "TCS"], days=2, force_refresh=False)
            ...
            with patch("market.replay.run.ReplayExecutor.__init__", mock_executor_init):
                ...
                self.assertTrue((Path(self.temp_dir) / "output" / "benchmark_report.html").exists())
                self.assertTrue((Path(self.temp_dir) / "output" / "benchmark_report.json").exists())
                self.assertTrue((Path(self.temp_dir) / "output" / "benchmark_report.csv").exists())
                
                # Check latest links folder contains output files
                self.assertTrue((Path(self.temp_dir) / "latest" / "benchmark_report.html").exists())
                
                # Check longitudinal history log was populated
                self.assertTrue((Path(self.temp_dir) / "benchmark_history.json").exists())
