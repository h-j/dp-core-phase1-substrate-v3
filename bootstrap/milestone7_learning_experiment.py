...
def run_learning_experiment():
    ...
    assert (
        g1["status"] == "PASS"
    ), f"Milestone 7 Learning Loop Gate 1 Temporal Isolation Violation: {g1['evidence']}"
    assert (
        g7["status"] == "PASS"
    ), f"Milestone 7 Learning Loop Gate 7 ERC Authorization Violation: {g7['evidence']}"
    assert (
        g8["status"] == "PASS"
    ), f"Milestone 7 Learning Loop Gate 8 Immutability Violation: {g8['evidence']}"
    ...
    summary = {
        ...
        "condition_c_influence_blocked": {
            "mean_compiled_candidates": (
                (cond_c_candidates_count / total_triggered)
                if total_triggered > 0
                else 0
            ),
            "mean_compilation_budget_spent": (
                (cond_c_comp_budget / total_triggered) if total_triggered > 0 else 0
            ),
            "mean_evidence_budget_spent": (
                (cond_c_ev_budget / total_triggered) if total_triggered > 0 else 0
            ),
        },
        "condition_d_influence_enabled": {
            "mean_compiled_candidates": (
                (cond_d_candidates_count / total_triggered)
                if total_triggered > 0
                else 0
            ),
            "mean_compilation_budget_spent": (
                (cond_d_comp_budget / total_triggered) if total_triggered > 0 else 0
            ),
            "mean_evidence_budget_spent": (
                (cond_d_ev_budget / total_triggered) if total_triggered > 0 else 0
            ),
        },
