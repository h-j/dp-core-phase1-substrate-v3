...
pivoted_precision = df.pivot(index=["world_id", "seed"], columns="condition", values="precision").reset_index()
pivoted_lift = df.pivot(index=["world_id", "seed"], columns="condition", values="mean_lift").reset_index()
...
for cond in agent_cols:
    ...
    pairwise_results[cond] = {
        "precision": compute_paired_stats(pivoted_precision, "Agent C (Evidence Representation)", cond),
        "mean_lift": compute_paired_stats(pivoted_lift, "Agent C (Evidence Representation)", cond)
    }
...
for world_id, w_group in df.groupby("world_id"):
    ...
    world_stats.append({
        ...
        "winner": "Agent C" if prec_diff > 0 else "Agent B" if prec_diff < 0 else "Tie"
    })
...
for world_id, config in WORLDS_CONFIG.items():
    ...
    for seed in range(1, 31):  # 30 seeds
        ...
        sel_max_signed = [e.proposition_id for e in sorted(evidence, key=lambda e: e.signed_lift, reverse=True)[:5]]
        
        # Policy: Max Absolute Lift
        sel_max_abs = [e.proposition_id for e in sorted(evidence, key=lambda e: e.absolute_lift, reverse=True)[:5]]
        
        # Policy: Max Activation Count (Frequency)
        sel_max_freq = [e.proposition_id for e in sorted(evidence, key=lambda e: e.activation_count, reverse=True)[:5]]
        
        # Policy: Shrinkage-adjusted Lift: signed_lift * (1.0 - uncertainty_score)
        sel_shrinkage = [e.proposition_id for e in sorted(evidence, key=lambda e: e.signed_lift * (1.0 - e.uncertainty_score), reverse=True)[:5]]
        ...
        if len(filtered) < 5:
            filtered = evidence
        sel_threshold = [e.proposition_id for e in sorted(filtered, key=lambda e: e.signed_lift, reverse=True)[:5]]
        
        eval_max_signed = evaluate_selection(sel_max_signed, candidates, evaluation_experiences, 501, 1000)
        eval_max_abs = evaluate_selection(sel_max_abs, candidates, evaluation_experiences, 501, 1000)
        eval_max_freq = evaluate_selection(sel_max_freq, candidates, evaluation_experiences, 501, 1000)
        eval_shrinkage = evaluate_selection(sel_shrinkage, candidates, evaluation_experiences, 501, 1000)
        eval_threshold = evaluate_selection(sel_threshold, candidates, evaluation_experiences, 501, 1000)
...
print("Forensic audit analysis completed. Results written to data/synthetic_experiment_results/s1_audit_data.json")
