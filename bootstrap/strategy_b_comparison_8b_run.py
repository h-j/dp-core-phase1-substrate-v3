...
def run_comparison():
    print(f"Running Multi-Case Strategy B Comparison on Llama3 (8B) for {len(SAMPLED_DAYS)} days...", flush=True)
    
    # Load baseline claims from findings of Epoch 3
    epoch3_path = Path("/Users/hemantj/.gemini/antigravity-ide/brain/face4af3-acf5-4ebb-a5bd-ccb6af890aa2/multi_case_comparison_findings.json")
    ...
    for day in SAMPLED_DAYS:
        ...
        snapshot_path = Path(f"/Users/hemantj/Proj/dp_core/dp-core-phase1-substrate-v3/data/replay_snapshots/reliance/run_20260701_123929/day_{day:04d}.json")
    ...
    output_path = Path("/Users/hemantj/.gemini/antigravity-ide/brain/face4af3-acf5-4ebb-a5bd-ccb6af890aa2/multi_case_comparison_findings_8b.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
        
    print(f"\nCompleted 8B comparison run. Generated results for {len(results)} days.", flush=True)
    
    # Compute aggregates
    total = len(results)
    compliant = len([r for r in results if r["schema_compliance"]])
    identical = len([r for r in results if r["semantic_preservation_class"] == "IDENTICAL_OR_EQUIVALENT"])
    minor_var = len([r for r in results if r["semantic_preservation_class"] == "MINOR_NON_MATERIAL_VARIATION"])
    distortion = len([r for r in results if r["semantic_preservation_class"] == "MATERIAL_DISTORTION"])
    
    print(f"\nLlama3 (8B) Comparison Metrics:", flush=True)
    print(f"Total cases evaluated: {total}", flush=True)
    print(f"Schema compliant: {compliant}/{total} ({compliant/total*100:.1f}%)", flush=True)
    print(f"Identical claims: {identical}/{total} ({identical/total*100:.1f}%)", flush=True)
    print(f"Minor non-material variation: {minor_var}/{total} ({minor_var/total*100:.1f}%)", flush=True)
    print(f"Material distortions: {distortion}/{total} ({distortion/total*100:.1f}%)", flush=True)
