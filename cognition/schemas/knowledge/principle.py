...
class FalsifiablePrediction(CognitionBase):
    ...
    applicability_filter: Dict[str, Any] = Field(default_factory=dict, description="Context conditions")
...
class Principle(CognitionBase):
    ...
    def trust_score(self) -> float:
        ...
        contradiction_term = self.contradiction_count * 4.0
        age_decay_term = self.stability_age * 0.05
        
        score = evidence_term + reuse_term + helpfulness_term + cross_asset_term - contradiction_term - age_decay_term
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "falsifiable_predictions": [p.to_dict() for p in self.falsifiable_predictions],
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Principle":
        ...
        inst = cls(
            ...
            falsifiable_predictions=[FalsifiablePrediction.from_dict(p) for p in data.get("falsifiable_predictions", [])],
            confidence=data.get("confidence", 0.5),
            support_count=data.get("support_count", 0),
            contradiction_count=data.get("contradiction_count", 0),
            created_at_step=data["created_at_step"],
            revision_history=[PrincipleRevision.from_dict(r) for r in data.get("revision_history", [])],
            ...
            confidence_adjustments_triggered=data.get("confidence_adjustments_triggered", 0),
