...
class WorldModel(CognitionBase):
    step: int
    narrative_summary: str = Field(description="Narrative text synthesis of current active forces")
    active_principle_ids: List[str] = Field(default_factory=list, description="IDs of active/stable principles")
    regime_constraints: Dict[str, Any] = Field(default_factory=dict, description="Deterministic constraints mapping regime to overrides")
    
    # Matured World Model fields
    dominant_mechanisms: List[Dict[str, Any]] = Field(default_factory=list, description="Mechanisms backed by principles")
    active_constraints: List[Dict[str, Any]] = Field(default_factory=list, description="Overriding constraints with references")
    supporting_canonical_principles: List[str] = Field(default_factory=list, description="List of canonical principle IDs")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorldModel":
        ...
        inst = cls(
            ...
            supporting_canonical_principles=data.get("supporting_canonical_principles", []),
