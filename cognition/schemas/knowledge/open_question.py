...
class OpenQuestion(CognitionBase):
    ...
    source_contradiction_ids: List[str] = Field(default_factory=list, description="Surprise/contradiction IDs that prompted the question")
    hypothesized_factors: List[str] = Field(default_factory=list, description="Hypothesized indicators/missing factors to test")
    status: QuestionStatus = Field(default=QuestionStatus.ACTIVE)
    resolution_principle_id: Optional[str] = Field(default=None, description="Principle ID that resolved the question")
