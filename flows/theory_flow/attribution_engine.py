...
class AttributionEngine:
    ...
    def attribute(
        ...
        theory_dict = theory.summary_structured.model_dump() if hasattr(theory, 'summary_structured') and theory.summary_structured else {}
        components = [MechanismComponent.from_dict(c) for c in theory_dict.get("mechanism_components", [])]
        ...
        for c in components:
            ...
            if "higher" in c.expected_behavior.lower() and "lower" in obs_text: failed.append(c.component_id)
            else: passed.append(c.component_id)
        
        outcome = "falsified" if (failed and len(failed) == len(components)) else "validated" if not failed else "partial"
        ...
        if failed and self.llm:
            ...
            try: attribution.attribution_reasoning = self.llm.generate(prompt)
            except: attribution.attribution_reasoning = "Causal narrative unavailable."
