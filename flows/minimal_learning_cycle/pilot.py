    ...
    def generate_world_dict(registry_entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if zone == "EVIDENCE_LIMITED":
            ...
            if zone == "CLEAR":
                ...
                expected_reason = "SUFFICIENT_POSITIVE_PROSPECTIVE_EFFECT" if true_effect > 0 else "SUFFICIENT_NEGATIVE_PROSPECTIVE_EFFECT"
            else:
...
class MLCPilotMetrics:
    ...
    def calculate_zone_metrics(
        ...
        oracle_map = {o["world_id"]: o.get("expected_decision", o.get("decision")) for o in oracle_decisions}
        ...
        mlc_correct_pooled = sum(1 for w_id, dec in mlc_map.items() if dec == oracle_map[w_id] and dec != "COMPILATION_REJECTED")
        b2_correct_pooled = sum(1 for w_id, dec in b2_map.items() if dec == oracle_map[w_id] and dec != "COMPILATION_REJECTED")
        b3_correct_pooled = sum(1 for w_id, dec in b3_map.items() if dec == oracle_map[w_id] and dec != "COMPILATION_REJECTED")
        b4_correct_pooled = sum(1 for w_id, dec in b4_map.items() if dec == oracle_map[w_id] and dec != "COMPILATION_REJECTED")
        ...
        results["pooled"] = {
            ...
            "mlc_accuracy": round(mlc_correct_pooled / n_pooled, 4) if n_pooled > 0 else 0.0,
            "b2_accuracy": round(b2_correct_pooled / n_pooled, 4) if n_pooled > 0 else 0.0,
            "b3_accuracy": round(b3_correct_pooled / n_pooled, 4) if n_pooled > 0 else 0.0,
            "b4_accuracy": round(b4_correct_pooled / n_pooled, 4) if n_pooled > 0 else 0.0,
            "mlc_vs_b2_difference": round((mlc_correct_pooled - b2_correct_pooled) / n_pooled, 4) if n_pooled > 0 else 0.0,
            "mlc_vs_b3_difference": round((mlc_correct_pooled - b3_correct_pooled) / n_pooled, 4) if n_pooled > 0 else 0.0,
        }

        # 2. Zone-level Accuracies
        for zone in zones:
            zone_world_ids = {entry["world_id"] for entry in registry if entry["zone"] == zone}
            
            mlc_correct = sum(1 for w_id in zone_world_ids if mlc_map.get(w_id) == oracle_map[w_id] and mlc_map.get(w_id) != "COMPILATION_REJECTED")
            b2_correct = sum(1 for w_id in zone_world_ids if b2_map.get(w_id) == oracle_map[w_id] and b2_map.get(w_id) != "COMPILATION_REJECTED")
            b3_correct = sum(1 for w_id in zone_world_ids if b3_map.get(w_id) == oracle_map[w_id] and b3_map.get(w_id) != "COMPILATION_REJECTED")
            b4_correct = sum(1 for w_id in zone_world_ids if b4_map.get(w_id) == oracle_map[w_id] and b4_map.get(w_id) != "COMPILATION_REJECTED")
            
            n_zone = sum(1 for w_id in zone_world_ids if mlc_map.get(w_id) != "COMPILATION_REJECTED")
            ...
            results[zone.lower() + "_zone"] = {
                ...
                "mlc_vs_b2_difference": round((mlc_correct - b2_correct) / n_zone, 4) if n_zone > 0 else 0.0,
                "mlc_vs_b3_difference": round((mlc_correct - b3_correct) / n_zone, 4) if n_zone > 0 else 0.0,
            }
    ...
    def calculate_h4_decomposition(
        ...
        oracle_map = {o["world_id"]: o.get("expected_decision", o.get("decision")) for o in oracle_decisions}
        oracle_reason_map = {o["world_id"]: o.get("expected_reason", o.get("reason_code")) for o in oracle_decisions}
        b2_map = {w_id: dec for w_id, dec in baseline_results["b2_decisions"].items()}
        mlc_map = {d["world_id"]: d["decision"] for d in decisions}
        ...
        return {
            "pooled": {
                ...
                "difference": round((b2_pooled_fa - mlc_pooled_fa) / n, 4) if n > 0 else 0.0
            },
            "false_admit_reject": {
    ...
    def calculate_h5_metrics(
        ...
        epistemic_decisions = [d for d in decisions if d["decision"] != "COMPILATION_REJECTED"]
        n = len(epistemic_decisions)

        # Defer precision and recall
        actual_defers = [d for d in epistemic_decisions if d["decision"] == "DEFER"]
        expected_defers = [w_id for w_id, dec in oracle_map.items() if dec == "DEFER" and w_id in {d["world_id"] for d in epistemic_decisions}]
        
        true_defers = [d for d in actual_defers if oracle_map.get(d["world_id"]) == "DEFER"]

        defer_precision = len(true_defers) / len(actual_defers) if actual_defers else 0.0
        defer_recall = len(true_defers) / len(expected_defers) if expected_defers else 0.0

        # Subtype accuracy calculations
        ev_lim_worlds = [w_id for w_id, reason in oracle_reason_map.items() if reason == "EVIDENCE_LIMITED" and w_id in {d["world_id"] for d in epistemic_decisions}]
        eff_amb_worlds = [w_id for w_id, reason in oracle_reason_map.items() if reason == "EFFECT_AMBIGUITY" and w_id in {d["world_id"] for d in epistemic_decisions}]

        correct_ev_lim = sum(1 for w_id in ev_lim_worlds if mlc_map.get(w_id) == "DEFER" and mlc_reason_map.get(w_id) in ["INSUFFICIENT_PROSPECTIVE_EVIDENCE", "INSUFFICIENT_PROSPECTIVE_COVERAGE", "FAILED_READINESS"])
        correct_eff_amb = sum(1 for w_id in eff_amb_worlds if mlc_map.get(w_id) == "DEFER" and mlc_reason_map.get(w_id) == "AMBIGUOUS_PROSPECTIVE_EFFECT")

        ev_lim_subtype_accuracy = correct_ev_lim / len(ev_lim_worlds) if ev_lim_worlds else 0.0
        eff_amb_subtype_accuracy = correct_eff_amb / len(eff_amb_worlds) if eff_amb_worlds else 0.0

        # Confusion Matrix (ADMIT, REJECT, DEFER)
        confusion_matrix = {
            "actual_admit": {"predicted_admit": 0, "predicted_reject": 0, "predicted_defer": 0},
            "actual_reject": {"predicted_admit": 0, "predicted_reject": 0, "predicted_defer": 0},
            "actual_defer": {"predicted_admit": 0, "predicted_reject": 0, "predicted_defer": 0},
        }
...
class MLCPilotPowerAnalysis:
    ...
    def run_power_analysis(
        ...
        evaluated_world_ids = [d["world_id"] for d in decisions if d["decision"] != "COMPILATION_REJECTED"]
        ...
        for h_id, diffs in [("H2", h2_diffs), ("H3", h3_diffs), ("H4", h4_diffs)]:
            ...
            if std_dev > 0.0:
                ...
                power_n500 = 1.0  # Zero variance implies perfect power to detect positive effect
...
class MLCPilotValidityGates:
    ...
    def run_pilot_gates(
        ...
        gate_status["PILOT_GATE_1_WORLD_COUNT"] = {
            "status": "PASS" if len(world_registry) == 100 and len(decisions) == 100 else "FAIL",
        ...
        boundary_count = sum(1 for entry in world_registry if entry["zone"] == "BOUNDARY")
        evidence_limited_count = sum(1 for entry in world_registry if entry["zone"] == "EVIDENCE_LIMITED")
        zone_pass = (clear_count == 30 and boundary_count == 50 and evidence_limited_count == 20)
        ...
        boundary_worlds = [entry for entry in world_registry if entry["zone"] == "BOUNDARY"]
        for entry in boundary_worlds:
            ...
            if eff == -0.08:
                if gt_dec != "REJECT": boundary_consistent = False
            elif eff in [-0.04, 0.00, 0.05, 0.12]:
                if gt_dec != "DEFER" or gt_sub != "EFFECT_AMBIGUITY": boundary_consistent = False
            elif eff in [0.16, 0.20]:
                if gt_dec != "ADMIT": boundary_consistent = False
            else:
        ...
        seeds_pass = all(len(set(seeds)) > 1 for seeds in seeds_by_effect.values()) and len(seeds_by_effect) == 7
        ...
        threshold_pass = (ADMIT_THRESHOLD == 0.15 and REJECT_THRESHOLD == -0.05 and REJECT_THRESHOLD < ADMIT_THRESHOLD)
        ...
        paired_pass = (len(mlc_world_ids) == 100 and len(b2_decisions) == 100 and len(b3_decisions) == 100 and 
                       mlc_world_ids == set(b2_decisions.keys()) and mlc_world_ids == set(b3_decisions.keys()))
        ...
        h4_keys = ["pooled", "false_admit_reject", "false_admit_evidence_limited", "false_admit_effect_ambiguous"]
        ...
        power_keys = ["H2", "H3", "H4"]
        power_fields = ["observed_mean_difference", "observed_variance", "power_at_n500", "required_n_for_80_power"]
        ...
        gate_status["PILOT_GATE_12_POWER_REPORT_COMPLETENESS"] = {
            ...
            "evidence": "All required power fields exist for H2, H3, and H4." if power_pass else "Some required power fields or hypotheses are missing."
        }
