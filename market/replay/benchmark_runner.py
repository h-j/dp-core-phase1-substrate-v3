...
class BenchmarkRunner:
    ...
    def __init__(self, assets_list: list, days: int, force_refresh: bool = False):
        ...
        # Paths
        self.output_dir = Path(__file__).parent.parent.parent / "market" / "replay" / "output" / "benchmarks" / self.benchmark_id
        self.latest_dir = Path(__file__).parent.parent.parent / "market" / "replay" / "output" / "benchmarks" / "latest"
        self.history_path = Path(__file__).parent.parent.parent / "market" / "replay" / "output" / "benchmarks" / "benchmark_history.json"
        
        self.output_dir.mkdir(parents=True, exist_ok=True)
        self.latest_dir.mkdir(parents=True, exist_ok=True)

    def verify_benchmark_consistency(self, benchmark_data: dict, run_executors: dict) -> list:
        ...
        for asset, executor in run_executors.items():
            ...
            if not report_json_path.exists():
                warnings.append(f"Consistency check failed: report.json missing for {asset}")
                continue
            ...
            try:
                ...
                warnings.append(f"Consistency check failed: Cannot read report.json for {asset}: {e}")
                continue
                
            # Find matching summary dict in benchmark_data
            summary_dict = next((a for a in benchmark_data["assets_summary"] if a["symbol"] == asset), None)
            if not summary_dict:
                warnings.append(f"Consistency check failed: Asset {asset} not found in aggregated summary.")
                continue
                
            # Validate Accuracy
            pred_analysis = executor.replay_analysis_engine.analyze().get("prediction_analysis", {}) if executor.replay_analysis_engine else {}
            ...
            if abs(replay_acc - bench_acc) > 0.01:
                warnings.append(f"Accuracy inconsistency for {asset}: Replay shows {replay_acc:.2f}%, Benchmark summary shows {bench_acc:.2f}%")
            ...
            if replay_debt != bench_debt:
                warnings.append(f"Knowledge Debt inconsistency for {asset}: Replay shows {replay_debt}, Benchmark summary shows {bench_debt}")
            ...
            if replay_principles != bench_principles:
                warnings.append(f"Principles Count inconsistency for {asset}: Replay shows {replay_principles}, Benchmark summary shows {bench_principles}")
    ...
    def run(self) -> dict:
        ...
        for asset, executor in run_executors.items():
            ...
            if not report_json_path.exists():
                print(f"[BenchmarkRunner] WARNING: report.json missing for {asset} at {report_json_path}")
                continue
            ...
            accuracy_confidence = (
                "High" if eval_count >= 30
                else "Medium" if eval_count >= 10
                else "Low"
            )
            ...
            assets_summary.append({
                "symbol": asset,
                "category": BENCHMARK_ASSETS.get(asset, {}).get("category", "Unknown"),
                "accuracy": float(metrics.get("accuracy", 0.0)),
                "calibration_error": float(1.0 - (knowledge_health.get("trust", 0.0) / 100.0)),
                "knowledge_debt": int(metrics.get("knowledge_debt", 0)),
                "memory_reuse": float(knowledge_health.get("reuse_rate", 0.0) / 100.0),
                "contradiction_pressure": float(metrics.get("contradiction_pressure", 0.0)),
                "principles_count": sum(report_data.get("knowledge_lifecycle", {}).values()),
            ...
            em = executor.replay_analysis_engine.external_metrics if executor.replay_analysis_engine else {}
            asset_failures = em.get("component_failure_counts", {})
            failures_by_asset[asset] = asset_failures
            for comp in asset_failures.keys():
                all_failures[comp] = all_failures.get(comp, 0) + (1 if asset_failures[comp] > 0 else 0)
                
            # Extract latest world model narrative and regime constraints
            world_models_comparison.append({
                "symbol": asset,
                "narrative": wm_data.get("narrative", "No world model narrative generated."),
                "stability": "Stable" if len(wm_data.get("supporting_principles", [])) > 2 else "Emerging",
        ...
        for comp, shared_count in all_failures.items():
            ...
            for asset in self.assets_list:
                failure_matrix[comp][asset] = failures_by_asset.get(asset, {}).get(comp, 0) > 0
            # Systemic definition: fails in >= 50% of the successfully replayed assets
            failure_matrix[comp]["systemic"] = shared_count >= max(2, len(self.assets_list) // 2)
        ...
        for asset, principles_list in all_principles_by_asset.items():
            for p in principles_list:
                ...
                target = preds[0].get("target_component", "general") if preds else "general"
        ...
        for target, occurrences in principles_by_component.items():
            ...
            if len(unique_assets) >= 2:
                ...
                best_statement = max(statements, key=len)  # Prefer the longest/most detailed one
        ...
        systemic_failures_count = sum(1 for m in failure_matrix.values() if m["systemic"])
        failure_similarity = (systemic_failures_count / total_failures_count) if total_failures_count > 0 else 0.0
        ...
        principle_similarity = (generalized_principles_count / unique_principles_count) if unique_principles_count > 0 else 0.0
        ...
        world_model_similarity = (shared_regimes / total_unique_regimes) if total_unique_regimes > 0 else 0.0

        # Component 4: Memory Behaviour (MB)
        memory_behaviour = float(np.mean([a["memory_reuse"] for a in assets_summary])) if assets_summary else 0.0
        ...
        avg_reuse = 0.0
        if assets_summary:
            avg_calibration = float(np.mean([a["calibration_error"] for a in assets_summary]))
        ...
        health_report = {
            ...
            "metrics_verified": 18,
            "aggregation_errors": len([w for w in warnings if "inconsistency" in w or "failed" in w]),
        ...
        current_summary = {
            ...
            "generalization_score": weighted_generalization_score,
            "avg_accuracy": float(np.mean([a["accuracy"] for a in assets_summary])) if assets_summary else 0.0,
            "git_commit": git_commit
        }
        ...
        print("\n" + "=" * 80)
        print(f"✓ MULTI-ASSET BENCHMARK REPORT COMPLETED SUCCESSFULLY: {self.benchmark_id}")
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=10, help="Max trading days to run for each benchmark asset"
    )
    parser.add_argument(
        "--assets",
        type=str,
    ...
    if invalid_assets:
        print(f"Error: The following assets are not configured in benchmark_settings: {', '.join(invalid_assets)}")
        sys.exit(1)
