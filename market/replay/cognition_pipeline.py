...
class CognitionPipeline:
    ...
    def _format_regime_context(self, matches: list) -> str:
        ...
        for match in matches[:3]:
            ...
            parts.append(
                f"{match.date} similarity {match.similarity:.2f}, confidence {match.confidence:.2f}{contradiction}"
            )
    ...
    def run_cognition(
        ...
        historical_context = self._format_historical_context(
            recent_validations, recent_reflections
        )
        ...
        if prior_dialectical_synthesis:
            max_sim = max(
                [getattr(m, "similarity", 0.0) for m in regime_matches] + [0.0]
            )
        ...
        reflection = self.reflection_flow.process(
            ...
            dialectical_synthesis=(
                self.dialectical_synthesizer.format_for_reflection(active_synthesis)
                if active_synthesis
                else None
            ),
        )
