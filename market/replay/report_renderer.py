...
class ReplayReportModel:
    ...
    def build_model(self) -> dict:
        ...
        # 2. Main Metrics
        pred_analysis = self.ae.analyze().get("prediction_analysis", {}) if self.ae else {}
        overall_acc = (
        ...
        knowledge_debt = int(em.get("knowledge_growth", {}).get("knowledge_debt_score", 0)) or int(em.get("knowledge_debt_score", 0))
        ...
        active_principles_count = sum(1 for p in all_principles if getattr(p, "status", "").lower() in ["active", "stable", "emerging", "trusted", "canonical"])
        ...
        knowledge_health = {
            "knowledge_debt_bar": min(100, int(knowledge_debt * 3.5)) if knowledge_debt > 0 else 10,
        ...
        knowledge_lifecycle = {
            "candidate": sum(1 for p in all_principles if getattr(p, "status", "").lower() == "candidate"),
            "emerging": sum(1 for p in all_principles if getattr(p, "status", "").lower() in ["active", "emerging"]),
            "trusted": sum(1 for p in all_principles if getattr(p, "status", "").lower() in ["stable", "trusted"]),
            "canonical": sum(1 for p in all_principles if getattr(p, "status", "").lower() == "canonical"),
            "retired": sum(1 for p in all_principles if getattr(p, "status", "").lower() == "retired"),
        }
        ...
        if self.ae:
            for i, r in enumerate(self.ae.prediction_history):
                ...
                prediction_str = f"{direction} ({confidence:.0%})" if direction != "N/A" else "Hold"
                ...
                knowledge_str = ", ".join(used_knowledge) if used_knowledge else "Experience Only"
        ...
        world_model = {
            "narrative": latest_wm.narrative_summary if latest_wm else "No world model generated yet.",
            ...
            "supporting_principles": [
                ...
                if self.repo.get_principle(p_id) else f"Principle {p_id[:8]}"
                for p_id in (latest_wm.active_principle_ids if latest_wm and latest_wm.active_principle_ids else [])
            ],
            "dominant_mechanisms": getattr(latest_wm, "dominant_mechanisms", []) if latest_wm else [],
            "active_constraints": getattr(latest_wm, "active_constraints", []) if latest_wm else [],
            "supporting_canonical_principles": getattr(latest_wm, "supporting_canonical_principles", []) if latest_wm else [],
            "confidence": getattr(latest_wm, "confidence", 0.5) if latest_wm else 0.5,
            "stability": getattr(latest_wm, "stability", "Emerging") if latest_wm else "Emerging",
            "evidence_count": getattr(latest_wm, "evidence_count", 0) if latest_wm else 0,
            "applicable_regimes": getattr(latest_wm, "applicable_regimes", []) if latest_wm else [],
            "open_questions": [
                {
        ...
        if self.executor.engine and hasattr(self.executor.engine, "data"):
            ...
            if len(df_replayed) > 0:
                chart_data["dates"] = df_replayed["date"].dt.strftime("%Y-%m-%d").tolist() if pd.api.types.is_datetime64_any_dtype(df_replayed["date"]) else df_replayed["date"].tolist()
                chart_data["close"] = df_replayed["close"].fillna(0.0).tolist()
                chart_data["delivery_pct"] = df_replayed["delivery_pct_5d"].fillna(0.0).tolist() if "delivery_pct_5d" in df_replayed.columns else (df_replayed["delivery_pct"].fillna(0.0).tolist() if "delivery_pct" in df_replayed.columns else [])
                chart_data["fii_net"] = df_replayed["fii_net"].fillna(0.0).tolist() if "fii_net" in df_replayed.columns else []
                chart_data["sector_zscore"] = df_replayed["sector_zscore"].fillna(0.0).tolist() if "sector_zscore" in df_replayed.columns else []
