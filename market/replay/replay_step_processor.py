...
class ReplayStepProcessor:
    ...
    def __init__(self, pipeline, persistence, simulator, engines, theory_lineage, contradiction_registry, regime_memory, regime_continuity_memory):
        self.pipeline = pipeline
        self.persistence = persistence
        self.simulator = simulator
        self.engines = engines # Dict of prediction, pressure, decision, horizon, epistemic
    ...
    def process_day(self, ctx: dict) -> CognitionResult:
        ...
        horizon_view = self.engines["horizon"].compute(ctx["run_market_observations"] + [market_obs])
        horizon_context = f"Horizon Context: {horizon_view.summary()}"

        # Regime Retrieval
        active_theory_count = self.theory_lineage.active_count() if self.theory_lineage else 0
        preliminary_sig = self.regime_memory.build_signature(
            date=date_str, observation=market_obs,
            confidence_values=ctx["confidence_history"][-10:] if ctx["confidence_history"] else [0.5],
            contradiction_severity=ctx["marker_severity"],
            active_theory_count=active_theory_count
        )
        regime_matches = self.regime_memory.retrieve(preliminary_sig, getattr(market_obs, "contradiction_markers", []))

        # 2. Cognition Pipeline
        regime_subtype = getattr(market_obs, "regime_subtype", "neutral")
        cog_payload = self.pipeline.run_cognition(
            day_idx=day_idx, market_obs=market_obs,
            ...
            horizon_context=horizon_context, regime_matches=regime_matches,
        ...
        # 3. Evolution & Scoring
        lineage_record = self._evolve_lineage(day_idx, cog_payload["abstraction"], theory)
        self._register_contradictions(day_idx, lineage_record, contradiction)
        
        # Retire/Revive
        self.theory_lineage.retire_stale_theories(day_idx, float(contradiction.get("score", 0.0)), lineage_record.id if lineage_record else None)
        self.theory_lineage.revive_matching_theories(getattr(cog_payload["abstraction"], "abstraction_summary", str(cog_payload["abstraction"])), day_idx)
        ...
        prediction = self.engines["prediction"].generate_prediction(
            ...
            volume_state=obs_data["derived"].get("volume_state", "normal"),
            momentum_regime=ctx["mom_regime"], volatility_regime=ctx["vol_regime"],
            volume_ratio_5d=float(obs_data["derived"].get("volume_ratio_5d", 1.0)),
        ...
        decisions = self.engines["decision"].evaluate(
            prediction_probe=prediction, transition_pressure=pressure,
            contradiction_score=float(contradiction.get("score", 0.0)),
            theory_usefulness=theory_usefulness.get("score", 0.0),
            confidence_state=None, date=date_str,
            volume_state=obs_data["derived"].get("volume_state", "normal"),
            participation_confirmation=getattr(market_obs, "participation_confirmation", "normal"),
        )
        ...
        self.simulator.record_day_result(
            date=date_str, prediction_direction=ctx["prior_prediction"].direction.value if ctx["prior_prediction"] else "uncertain",
            prediction_confidence=ctx["prior_prediction"].confidence if ctx["prior_prediction"] else 0.0,
            actual_daily_return_pct=obs_data["derived"]["daily_return_pct"] if obs_data.get("derived") else 0.0,
            market_daily_return_pct=obs_data["derived"]["daily_return_pct"] if obs_data.get("derived") else 0.0,
            decisions=decisions
        )

        conf_state = self.engines["confidence_evolution"].evolve(
            confidence_state=theory.confidence_state, validation=cog_payload["validation"],
            reflection=reflection, contradiction_result=contradiction,
            market_observation=market_obs, recent_validations=ctx["run_validations"][-5:],
        ...
        final_sig = self.regime_memory.build_signature(
            date=date_str, observation=market_obs, confidence_values=ctx["confidence_history"][-10:],
            contradiction_severity=float(contradiction.get("score", 0.0)),
            active_theory_count=self.theory_lineage.active_count() if self.theory_lineage else active_theory_count
        )
        self.regime_memory.persist(
            step=day_idx, signature=final_sig, active_theories=self.theory_lineage.active_theories(),
            contradictions=contradiction.get("indicators", []), confidence=conf_state.empirical_confidence
        )

        # Result Assembly
        result = CognitionResult(
            day_index=day_idx, date=date_str, observation=market_obs,
            abstraction=cog_payload["abstraction"], theory=theory, contradiction=contradiction,
            reflection=reflection, confidence_state=conf_state, validation=cog_payload["validation"],
            theory_usefulness=theory_usefulness, transition_pressure=pressure, prediction=prediction,
            prior_prediction_result=ctx["prior_prediction_result"], regime_matches=regime_matches,
            horizon_view=horizon_view, regime_signature=final_sig, regime_history=self.regime_continuity_memory.summary(regime_subtype),
            decisions=decisions, branch_stats=cog_payload["branch_stats"], lesson=ctx.get("lesson", ""),
            epistemic_quality={"theory": evaluate_epistemic_quality(theory.summary_structured.claim if theory.summary_structured else theory.summary)}
        )
        
        return result

    def _evolve_lineage(self, day_idx, abstraction, theory):
        if not self.theory_lineage: return None
        abs_text = getattr(abstraction, "abstraction_summary", str(abstraction))
        res = self.theory_lineage.evolve_theory(
            abstraction=abs_text, step=day_idx,
            confidence_state={
    ...
    def _register_contradictions(self, day_idx, record, contradiction):
        if not self.contradiction_registry or not record: return
        indicators = contradiction.get("indicators", [])
        self.contradiction_registry.register_contradictions(
            theory_id=record.id, descriptions=indicators,
            severity=float(contradiction.get("score", 0.0)), step=day_idx
        )
        if indicators:
            self.theory_lineage.record_contradictions(tid=record.id, descriptions=indicators, step=day_idx)