...
class MarketContradictionMapper:
    ...
    def detect_zones(self, market_observation, market_outcome, prior_theory):
        """Detect contradiction zones from market behavior."""

        zones = []

        # Price strength vs weak breadth
        if self._detect_price_breadth_divergence(market_observation, market_outcome):
            zones.append(
                {
                    ...
                    "severity": self._calc_price_breadth_severity(
                        market_observation, market_outcome
                    ),
            ...
            zones.append(
                {
                    ...
                    "severity": self._calc_momentum_volatility_severity(market_outcome),
                    "timestamp": datetime.now(UTC),
                }
            )

        # Liquidity concentration vs participation
        if self._detect_liquidity_participation_gap(market_observation, market_outcome):
            zones.append(
                {
        ...
        if self._detect_trend_reversal_zone(
            market_observation, market_outcome, prior_theory
        ):
            zones.append(
                {
        ...
        # Update history
        for zone in zones:
            self.contradiction_history[zone["zone"]].append(zone["timestamp"])
    ...
    def map_recurring_zones(self, lookback_hours: int = 168) -> dict:
        ...
        # Sort by frequency
        sorted_zones = sorted(
            zone_frequencies.items(), key=lambda x: x[1]["occurrences"], reverse=True
        )

        return {
            "recurring_zones": dict(sorted_zones),
            "hotspot_count": len(sorted_zones),
            "most_severe": (sorted_zones[0] if sorted_zones else None),
        }
    ...
    def _detect_price_breadth_divergence(
        self, market_observation, market_outcome
    ) -> bool:
        ...
        trend_strong = any(word in trend_text for word in ["higher", "strong", "up"])

        breadth_weak = any(
            word in breadth_text for word in ["weak", "narrow", "divergence", "limited"]
        )

        return trend_strong and breadth_weak

    def _calc_price_breadth_severity(self, market_observation, market_outcome) -> float:
    ...
    def _calc_momentum_volatility_severity(self, market_outcome) -> float:
    ...
    def _detect_liquidity_participation_gap(
        ...
        liquidity_strong = any(
            word in liquidity_text for word in ["ample", "strong", "concentrated"]
        )

        participation_weak = any(
            word in breadth_text for word in ["weak", "narrow", "selective"]
        )

        return liquidity_strong and participation_weak

    def _calc_liquidity_participation_severity(self, market_outcome) -> float:
    ...
    def _detect_trend_reversal_zone(
        self, market_observation, market_outcome, prior_theory
    ) -> bool:
        ...
        outcome_shows_reversal = any(
            word in reversal_text for word in ["reversal", "correction", "reversed"]
        )
