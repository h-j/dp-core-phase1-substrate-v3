...
class NSEFetcher:
    ...
    def __init__(self, cache_dir: Path = None):
        if cache_dir:
            ...
            self.cache_dir = Path(__file__).parent.parent.parent / "data" / "market_data" / "delivery"
    ...
    def fetch_delivery_data(
        ...
        if cache_path.exists() and not force_refresh:
            try:
                ...
                logger.info(f"Loaded {len(df)} delivery records from cache for {base_symbol}")
                return df
            except Exception as e:
            ...
            for attempt in range(retries):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        logger.warning(f"Received 429 rate limit. Backing off for {backoff}s...")
                        time.sleep(backoff)
                        backoff *= 2.0
                    else:
                        logger.warning(f"NSE API returned status {res.status_code} on attempt {attempt+1}")
                        time.sleep(backoff)
                        backoff *= 2.0
                except Exception as e:
        ...
        if response_text:
            try:
                ...
                delivery_col = next((c for c in df.columns if "dly" in c.lower() or "deliver" in c.lower()), None)

                if date_col and delivery_col:
                    cleaned_df = df[[date_col, delivery_col]].copy()
                    cleaned_df.columns = ["date", "delivery_pct"]
                    cleaned_df["date"] = pd.to_datetime(cleaned_df["date"])
                    cleaned_df["delivery_pct"] = pd.to_numeric(cleaned_df["delivery_pct"], errors="coerce")
                    cleaned_df = cleaned_df.dropna().sort_values("date")
                    
                    # Cache to parquet
                    cleaned_df.to_parquet(cache_path, index=False)
                    logger.info(f"Cached {len(cleaned_df)} delivery records to {cache_path}")
                    return cleaned_df
                else:
                    logger.error(f"Could not identify required columns in NSE response. Columns: {list(df.columns)}")
            except Exception as e:
                logger.error(f"Error parsing NSE response: {e}")

        # Fallback 1: Load from existing cache if available (offline continuation)
        if cache_path.exists():
            logger.warning("Live fetch failed. Continuing offline with cached delivery data.")
            try:
                df = pd.read_parquet(cache_path)
                df["date"] = pd.to_datetime(df["date"])
                return df
        ...
        logger.warning(f"No cache found and live fetch failed. Generating simulated delivery data for {base_symbol}")
