...
class MoneycontrolFetcher:
    ...
    def __init__(self, cache_dir: Path = None):
        if cache_dir:
            ...
            self.cache_dir = Path(__file__).parent.parent.parent / "data" / "market_data" / "fii_dii"
    ...
    def fetch_fii_dii_data(
        ...
        if self.cache_file.exists() and not force_refresh:
            try:
                ...
                logger.error(f"Failed to read cached FII/DII Parquet {self.cache_file}: {e}")
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if fii_dii_table is not None:
                    ...
                    logger.error("FII/DII table not found in retrieved tables on Moneycontrol")
            else:
                logger.error(f"Failed to hit Moneycontrol, status: {res.status_code}")
        except Exception as e:
            logger.error(f"Moneycontrol scraper failed: {e}. Page layout might have changed.")
        ...
        if fetched_df is not None and not fetched_df.empty:
            if self.cache_file.exists():
                try:
                    ...
                    combined = pd.concat([existing_df, fetched_df]).drop_duplicates(subset=["date"], keep="last")
                    combined = combined.sort_values("date")
                    combined.to_parquet(self.cache_file, index=False)
                    logger.info(f"Merged and updated FII/DII cache: {len(combined)} records total")
                    return combined
                except Exception as e:
        ...
        if self.cache_file.exists():
            logger.warning("Live fetch failed. Continuing offline with cached FII/DII data.")
            try:
                df = pd.read_parquet(self.cache_file)
                df["date"] = pd.to_datetime(df["date"])
                return df
        ...
        logger.warning("No cache found and live fetch failed. Generating simulated FII/DII flow data.")
    ...
    def _clean_moneycontrol_table(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not date_col or not fii_net_col or not dii_net_col:
            ...
            logger.warning("Explicit column naming mismatch. Falling back to index-based cleaning.")
            if len(df.columns) >= 3:
